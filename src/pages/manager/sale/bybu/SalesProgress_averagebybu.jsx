...
export default function SalesComparisonProgressAveragebybu({bu}) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        setTarget(Number(res.data.total_avg.target) || 0);
        setRevenue(Number(res.data.total_avg.revenue) || 0);
        setLastYear(Number(res.data.total_avg.last_year) || 0);
      })
      .catch((err) => {
  ...
  return (
    <div className="card shadow-sm border-0 p-4 bg-white rounded-4">
      <center>
        <h5 className="mb-4 text-danger fw-bold ">📈 ປຽບທຽບຍອດຂາຍສະສົມ</h5>
      </center>
      ...
      <div>
        <div className="d-flex justify-content-between">
          <span>📅  1-ເດືອນນີ້ປີກ່ອນ</span>
          <span className="fw-bold text-danger">
            {formatNumber(lastYear)} ກີບ ({percentLastYear.toFixed(1)}%)
          </span>
        </div>
        <div className="progress bg-light" style={{ height: '10px' }}>
          <div
