...
const ComparisonCard = ({ title, data, icon, color = '#28a745', barColor = '#ffc107' }) => {
...
export default function SalesComparisonProgressAllBU({ bu }) {
    ...
    console.log("data lastmonth", data)
    ...
    return (
        <div className="grid lg:grid-cols-4 ">
            <div className="w-full sm:w-full px-2">
                <ComparisonCard title="📅 ເດືອນນີ້" color='#52c41a' data={data.total_month} />
            </div>
            <div className="w-full sm:w-full px-2 ">
                <ComparisonCard title="📅 ເດືອນກອ່ນ" color='#fa8c16' data={data.lastMonth} />
            </div>
            <div className="w-full sm:w-full px-2">
                <ComparisonCard title="📦 ສະສົມ" color='#722ed1' data={data.total_avg} />
            </div>
            <div className="w-full sm:w-full px-2 ">
                <ComparisonCard title="📈 ທັງປີ" color='#f5222d' data={data.total_year} />
            </div>
        </div>
    );
}
